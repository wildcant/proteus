#!/usr/bin/env bash
# The one Lingui watcher of a dev session: backend recompiles `locales/*.ts`, store and admin re-extract their
...
# SIGTERM, so terminating it leaves every `lingui --watch` process running. `set -m` gives each
# watcher a process group of its own, and the trap signals the whole group — pnpm, lingui and the
# subcommand lingui spawns — on INT, TERM, HUP, or any watcher exiting.
...
pgids+=($!)
pnpm --silent --filter admin run i18n:watch &
pgids+=($!)

# Any watcher dying ends the session, so a pane that looks alive is never watching half the
# catalogs. A poll rather than `wait -n`, which the bash macOS ships (3.2) does not have.
while kill -0 "${pgids[0]}" 2>/dev/null && kill -0 "${pgids[1]}" 2>/dev/null && kill -0 "${pgids[2]}" 2>/dev/null; do
  sleep 1
done
