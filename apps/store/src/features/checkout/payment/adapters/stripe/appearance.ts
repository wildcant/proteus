...
export function appearanceFor(tokens: ThemeTokens): Appearance {
  return {
    ...
    rules: {
      // The accordion item, written as the twin of `ROW_CLASS` in `saved-card-row.tsx`: one
      // hairline, square, 16px in, no shadow. Both sides are the same row drawn in two places we
      // cannot share a stylesheet across, so a change to one is a change to the other.
      '.AccordionItem': {
