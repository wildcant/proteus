 ...
 * through a rendered form. Our own copy is a `msg` descriptor, translated with the caller's
 * per-request `i18n`; Stripe's `message` already follows the Elements `locale`.
 */

/** What every failure we cannot explain says. Never a gateway string. */
export const GENERIC_FAILURE_MESSAGE: MessageDescriptor = msg`We could not process your payment. Please try again or use a different card.`

/** The one string every issuer decline collapses to, except the four a shopper can act on. */
const DECLINED_MESSAGE: MessageDescriptor = msg`Your card was declined. Please try another card or contact your bank.`

const AUTHENTICATION_FAILED_MESSAGE: MessageDescriptor = msg`We could not authenticate your card with your bank. Please try again or use a different card.`
 ...
 */
export function customerMessageForStripeError(error: StripeFailure | null | undefined, i18n: I18n): string {
  if (!error) return i18n._(GENERIC_FAILURE_MESSAGE)

  if (error.code === 'payment_intent_authentication_failure') return i18n._(AUTHENTICATION_FAILED_MESSAGE)

  if (error.type === 'card_error') {
    if (error.code !== ISSUER_DECLINE_CODE) return error.message ?? i18n._(DECLINED_MESSAGE)

    const declineCode = error.decline_code
    if (declineCode && ACTIONABLE_DECLINE_CODES.has(declineCode)) return error.message ?? i18n._(DECLINED_MESSAGE)
    // Every other decline — named, unnamed, and every code Stripe adds after this is written.
    return i18n._(DECLINED_MESSAGE)
  }

  if (error.type === 'validation_error') return error.message ?? i18n._(GENERIC_FAILURE_MESSAGE)
  ...
  return i18n._(GENERIC_FAILURE_MESSAGE)
}
