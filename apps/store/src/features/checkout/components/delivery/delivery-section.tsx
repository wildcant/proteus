...
type DeliverySectionProps = Pick<CheckoutData, 'cart' | 'addresses' | 'isAddressesLoading' | 'customer'> & {
  form: CheckoutForm
}

/** Confirm a saved address, or type one. A shopper is only ever offered one of the two. */
function Delivery({ cart, addresses, isAddressesLoading, form, customer }: DeliverySectionProps) {
  if (isAddressesLoading) return <Skeleton className="h-40 w-full" />
  if (addresses.length > 0 && customer) {
    return <ShppingAddressPicker form={form} cart={cart} addresses={addresses} customer={customer} />
  }
