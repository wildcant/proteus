...
type FeatureEntry = { id: string; title: string; location: string }
  ...
  const visit = (node: ts.Node) => {
    if (
      ...
      if (ts.isStringLiteral(keyArg)) {
        ...
        moduleKey = Modules[keyArg.name.text as keyof typeof Modules]
      }
          ...
          if (
            ...
            for (const element of prop.initializer.elements) {
              if (ts.isObjectLiteralExpression(element)) {
                ...
                if (moduleKey && !id.startsWith(`${moduleKey}.`)) {
                  problems.push({
                    location,
                    message: `feature id "${id}" is not prefixed with its module "${moduleKey}"`,
                    remedy: `rename the id to "${moduleKey}.<action>" — the module a permission belongs to is read back off this prefix`,
                  })
                  continue
                }

                features.push({ id, title, location })
              }
            }
          }
        }
  ...
  for (const feature of features) {
    ...
    if (first) {
      problems.push({
        ...
        message: `duplicate feature id "${feature.id}" — also declared at ${first.location}`,
...
function render(features: FeatureEntry[]): string {
  const entries = features.map((f) => `  { id: '${f.id}', title: '${f.title}' },`).join('\n')
