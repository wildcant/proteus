...
# localhost. Assumes `pnpm db:start` is up — the proxy itself is declared in docker-compose.yml.
#
#   pnpm --filter backend run dev:slow
#   DB_LATENCY_MS=200 pnpm --filter backend run dev:slow
set -euo pipefail
...
curl -sf -X POST "$TOXICS" -H 'Content-Type: application/json' \
  ...
  >/dev/null || { echo "No toxiproxy on :8474 — run pnpm --filter backend run db:start"; exit 1; }
