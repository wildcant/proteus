 ...
 * the step it was replaying to is done, the handler is *abandoned*: `src/framework/workflows/temporal/replay.ts` hands
 ...
 * here either — `src/framework/workflows/temporal/replay.ts` asserts that at runtime, where it can see two `ctx.step`
