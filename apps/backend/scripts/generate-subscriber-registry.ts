...
function render(subscribers: Discovered[]): string {
  ...
  return `// GENERATED by \`npm run subscribers:generate\`. Do not edit — your change will be overwritten.
//
...
if (checking) {
  if (existing !== rendered) {
    ...
    console.info(`    ${YELLOW}→${RESET} run \`npm run subscribers:generate\` and commit the result`)
    console.info('')
    process.exit(1)
  }
  console.info(
