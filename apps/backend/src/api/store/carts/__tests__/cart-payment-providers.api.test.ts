...
test.describe('GET /store/carts/:id/payment-providers', () => {
  test('offers exactly what the cart’s market carries, not every enabled provider', async ({
    ...
    expect(body.paymentProviders).toEqual([
      { id: STRIPE_PROVIDER_ID, isEnabled: true, label: 'Stripe', isTestOnly: false, publicConfig: expect.any(Object) },
    ])
  })
 ...
 *
 * Carried over from the flat `GET /store/payment-providers` this route replaced: scoping the list
 * to the cart's market changed which providers are answered with, not what a provider is allowed
 * to say about itself. The assertion is not "no secret is present" — which passes for as long as
 * nobody adds one — but "these keys and no others". An implementation that spread `StripeOptions`
 * would answer with `apiKey` and `webhookSecret` here and fail on the key set before anyone had to
 * think of the secret's name.
 */
const ALLOWED_PUBLIC_CONFIG_KEYS: Record<string, string[]> = {
  [STRIPE_PROVIDER_ID]: ['publishableKey'],
  [MANUAL_PROVIDER_ID]: [],
}
...
test.describe('GET /store/carts/:id/payment-providers public config', () => {
  /** A market carrying both providers, so one response covers every provider in the deployment. */
  const cartOfferingEverything = async (factories: Factories, service: Services) => {
    const unitedStates = await factories.create.region({ name: 'United States', currencyCode: 'usd' })
    await offer(factories, unitedStates, STRIPE_PROVIDER_ID)
    await offer(factories, unitedStates, MANUAL_PROVIDER_ID)
    return cartIn(service, unitedStates)
  }
  ...
  test('serves each provider only the keys that provider is allowed to publish', async ({
    ...
    const cart = await cartOfferingEverything(factories, service)
  ...
  test('serves the publishable key the storefront boots Stripe.js with', async ({ expect, factories, service }) => {
    const cart = await cartOfferingEverything(factories, service)

    const { body } = await providersFor(cart.id)

    const stripe = body.paymentProviders.find((provider) => provider.id === STRIPE_PROVIDER_ID)
  ...
  test('gives a provider with nothing publishable an empty object rather than omitting the field', async ({
    ...
    const system = body.paymentProviders.find((provider) => provider.id === MANUAL_PROVIDER_ID)
