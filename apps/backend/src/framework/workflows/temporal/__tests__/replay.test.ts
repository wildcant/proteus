...
test.describe('replay', () => {
  ...
  test('abandons a handler that wrapped a step in try/catch, where the simple adapter recovers', async ({ expect }) => {
     ...
     * here so it cannot drift silently. `workflow-wraps-a-step-in-try` rejects this shape and
     * `framework/workflows/README.md` documents it; this is the evidence behind both.
     *
