...
workflow up by name in `src/framework/workflows/temporal/registry.ts`, re-executes its handler from the top with a
...
imports from `src/framework/workflows/temporal/` or `src/framework/temporal/` today and should keep it — a feature that cannot be expressed without
...
`src/framework/workflows/temporal/__tests__/nested-workflow.server.test.ts` pins that shape; ADR-0021 records it as a residual.

`check:structure` enforces the boundary — `no-temporal-in-workerd` fails if `src/index.workerd.ts` can
reach `@temporalio/*`, `src/framework/temporal/` or `src/framework/workflows/temporal/` at all.
...
`src/framework/workflows/temporal/__tests__/` — `replay.test.ts` for the replay mechanism with no server,
...
Temporal; the rest are engine-blind — `src/framework/workflows/temporal/__tests__` and the other files here build their
...
evidence about a shape that is not deployed. `src/framework/workflows/temporal/__tests__/nested-workflow.server.test.ts` covers
