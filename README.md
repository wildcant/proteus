...
![Backend dependency graph](apps/backend/structure/dependency-graph.svg)
...
![Store unidirectional architecture](apps/store/structure/unidirectional-codebase.png)
...
`Tasks: Terminate Task` → `All Running Tasks` stops the seven panes and leaves Docker up, which is
