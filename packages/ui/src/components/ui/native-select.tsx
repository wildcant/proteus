...
type NativeSelectProps = Omit<React.ComponentProps<'select'>, 'size'> & {
  size?: 'sm' | 'default'
  /**
   * The positioning box that holds the select and its chevron. Only reach for this to change the
   * box itself — its width, or where the chevron sits inside it.
   */
  wrapperClassName?: string
}

/**
 * `className` lands on the `<select>`, not on the wrapper: the control is what a caller means when
 * it passes `rounded-none` or `h-11`, and routing those to the wrapper silently dropped them.
 */
function NativeSelect({ className, wrapperClassName, size = 'default', ...props }: NativeSelectProps) {
  return (
    <div
