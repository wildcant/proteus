...
**Blocked by:** nothing. 01 and 03 have landed; this is the frontend half they left behind.
...
`AGENTS.md` is 290 lines / 20,886 bytes. The whole `## Frontend Apps` section is 2,304 of them,
...
| Lead ¶3 — Orval clients and the `qs` fetcher | 300 | **stays in root** — true of both apps; duplicating it into two files is two things to keep in step |
| ¶3's last sentence — backend-as-library | −76 | **deleted** — see Notes |
...
**Net: 1,487 bytes out, about 180 back in — roughly 1,307 bytes, ~465 tokens, 6.3% of the file.** The
earlier figure of 1,443 assumed all of ¶2 could move and undercounted the three subsections; the two
errors ran in opposite directions. **The tokens were never the reason to do this** — closing the
defect above is.
...
- [ ] **¶2 is split three ways, not moved.** ¶2a and ¶2b are gone from root and land in no nested
      file; ¶2c stays in root, unscoped. ADR-0013 stays referenced from ¶2c
...
**¶3 loses its last sentence, and does not gain it back.** *"The store additionally calls the
backend as a library from server functions"* describes a deprecated pattern with no live consumer;
the spec's *Backend-as-library is removed* section takes the code out. Do not carry that sentence
into either nested file, and do not wait for the removal to land — the sentence is already wrong.
...
**Adjacent, deliberately out of scope.** Reading bulletproof-react against the tree turned up four
structural gaps — the admin has no cross-feature import rule at all (149 import sites, two cycles:
`products ↔ product-options` and `regions ↔ store`), `src/`'s top level has no closed vocabulary,
filenames are kebab-case by habit with nothing checking it, and `no-loose-feature-files` carries an
`index.ts` carve-out that no feature uses. Those are checks, which §4.1 rates far above anything in
this ticket — but they are a separate ticket with a separate migration, and folding them in would
turn a bounded subtraction into an open-ended extraction.
