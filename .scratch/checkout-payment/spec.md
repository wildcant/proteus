...
- ~~The webhook route acknowledges the gateway immediately and defers processing, so that a webhook arriving before the shopper's own checkout request does not race it. The deferral interval and retry count are configurable, defaulting to the reference implementation's values.~~ **Dropped before merge.** The interim runner that gave this — an in-process `DeferredTasks` with a delay, a per-session queue and a retry ladder — was written, reviewed and then removed rather than merged: the event bus is the next piece of work and would have replaced it immediately, and a second mechanism doing the same job in the meantime is a migration nobody needs. Processing is inline until the event bus lands, so the race this described is open and carries a `TODO(events)` on the route. What survives from the exercise is the finding that made it worth having: the adapter already retries a transient gateway failure under one idempotency key, so the ladder above it only ever covered failures the adapter had given up on — which is Stripe's redelivery to handle, not ours.
...
- **A general-purpose event bus module and a locking module.** See *Webhook deferral*. This spec no longer builds the deferral behaviour either: it is deferred wholesale to the event bus, which is the next piece of work.
