...
**What to build:** The complete Playwright setup for the admin app and the first passing E2E test suite. After this ticket, running `pnpm --filter admin run test:e2e` starts the backend test server and admin Vite dev server, executes auth tests covering login, logout, invalid credentials, and unauthenticated redirect, and reports results.
...
- [ ] Verify: `pnpm --filter admin run test:e2e` runs all auth tests and they pass
